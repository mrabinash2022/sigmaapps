...
export default function MyTicketsScreen({ navigation }) {
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const { tickets: list } = await api.getMyTickets();
      setTickets(list || []);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <ScreenLayout>
      <View style={styles.container}>
        <Text style={styles.heading}>My support tickets</Text>
        <Text style={styles.sub}>Track issues you raised about your orders</Text>
          ...
          renderItem={({ item }) => (
            ...
            >
              <View style={styles.cardTop}>
                <Text style={styles.cardTitle}>{item.shop?.name || 'Shop'}</Text>
              ...
              <Text style={styles.preview} numberOfLines={2}>
                {item.messages?.[item.messages.length - 1]?.body || item.customerMessage}
              </Text>
              <TouchableOpacity
